  ...
  return (
    <div className="App">
      ...
      <div className='gallery'>{posts.length == 0 ? "No Posts Yet" : posts.filter(post => post.title.toLowerCase().includes(search.toLowerCase()) || search == '').map((post) => {
        // date = new Date()
        return (
          <Link to={`/post-detail/${post.id}`} state={{created_at: post.created_at, title: post.title, content: post.content, url: post.image_url, upvotes: post.upvotes, comments: post.comments}} style={{color: 'black', 'textDecoration': 'none'}}>
